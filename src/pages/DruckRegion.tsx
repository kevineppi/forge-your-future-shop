              ...
              <div className="grid lg:grid-cols-2 gap-16 items-center">
                <AnimatedSection animation="fade-in">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mb-10">
                    <Button asChild size="lg" variant="hero" className="text-base"><Link to="/kontakt">{ctaConfig.ctaLabel}<ArrowRight className="ml-2 w-5 h-5" /></Link></Button>
                    <Button asChild size="lg" variant="outline" className="group"><Link to="/fdm-3d-druck">FDM-Verfahren im Detail<ArrowRight className="ml-2 w-4 h-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200" /></Link></Button>
                  </div>
            ...
            <div className="container mx-auto px-4">
              <div className="flex flex-wrap gap-2 justify-center max-w-4xl mx-auto">
                ...
                {Object.values(regionalDruckData).filter(r => r.slug !== regionData.slug).slice(0, 12).map(r => (
                  <Link key={r.slug} to={`/3d-druck-${r.slug}`} onClick={() => window.scrollTo(0, 0)} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-card border border-border/60 hover:border-primary/40 hover:text-primary text-sm font-medium transition-all">
